...
//    private Topic updateDestinaion;
//    @Resource(name = "itemDeleteTopic")
//    private Topic deleteDestination;
    @Autowired
    private Topic itemAddTopic;
    @Autowired
    private Topic itemUpdateTopic;
    @Autowired
    private Topic itemDeleteTopic;
    @Override
    ...
    public OnlinStoreResult addItem(TbItem tbItem, String desc) {//form表单中提供的字段与接收的pojo中的字段对应上即可正确接收
        ...
        //向activeMq发送商品添加信息
        jmsTemplate.convertAndSend(itemAddTopic, Long.toString(itemId));
...
//        int i = 1/0;
        ...
        jmsTemplate.convertAndSend(itemUpdateTopic, Long.toString(itemId));
        return OnlinStoreResult.ok();
    }
    ...
    public OnlinStoreResult deleteItem(final long itemId) {
        ...
        jmsTemplate.convertAndSend(itemDeleteTopic, Long.toString(itemId));
        return OnlinStoreResult.ok();
    }
